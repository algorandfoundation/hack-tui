...
func Test_GetMetrics(t *testing.T) {
	metrics, err := GetMetrics("https://mainnet-api.4160.nodely.dev", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
	if err == nil {
		t.Fatal(err)
	}

	// TODO: ensure localnet is running before tests
	metrics, err = GetMetrics("http://localhost:4001", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
	if err != nil {
		t.Fatal(err)
	}
